...
describe('SW message dispatcher (F-SEC-01 — Object.hasOwn whitelisting)', () => {
  function makeMinimalHandlers(): HandlerRegistry {
    return {
      ...
      USDT0_QUOTE_BRIDGE: vi.fn(async () => ({ fee: '0' })) as any,
      USDT0_BRIDGE: vi.fn(async () => ({ hash: '0x', fee: '0' })) as any,
      ...
      ERC4337_QUOTE_SEND: vi.fn(async () => '0') as any,
      ERC4337_SEND: vi.fn(async () => ({ hash: '0x', fee: '0' })) as any,
      SPARK_GET_ADDRESS: vi.fn(async () => 'spark1example') as any,
      SPARK_GET_BALANCE: vi.fn(async () => '0') as any,
      SPARK_SEND: vi.fn(async () => 'sparktxhash') as any,
      SPARK_GET_DEPOSIT_ADDRESS: vi.fn(async () => 'bc1deposit') as any,
      SPARK_QUOTE_WITHDRAW: vi.fn(async () => ({ quoteId: 'q1', exitSpeed: 'MEDIUM' as const, userFeeSats: 0, l1BroadcastFeeSats: 0, totalFeeSats: 0 })) as any,
      SPARK_WITHDRAW: vi.fn(async () => ({ id: 'exit1', status: 'PENDING', feeSats: 0 })) as any,
      LIGHTNING_CREATE_INVOICE: vi.fn(async () => 'lnbc1example') as any,
      LIGHTNING_PAY_INVOICE: vi.fn(async () => 'req_1') as any,
      ...
      BACKUP_EXPORT_VAULT: vi.fn(async () => ({ backup: 'enc' })) as any,
      BACKUP_IMPORT_VAULT: vi.fn(async () => ({ ok: true as const })) as any,
    };
  }
